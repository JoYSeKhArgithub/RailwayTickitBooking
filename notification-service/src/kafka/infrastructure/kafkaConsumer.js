...
export class KafkaConsumer {
    constructor(kafkaClient, groupId = 'notification-service-group') {
    ...
    async subscribe({ topics, fromBeginning = false }) {
        ...
        logger.info('Subscribed to Kafka topics', { topics: topicList, groupId: this.groupId });
    }

    async run(options) {
        await this.connect();
        // Support both run(eachMessageFn) and run({ eachMessage: eachMessageFn, ... })
    ...
    async disconnect() {
        ...
        this.isConnected = false;
        logger.info('Kafka consumer disconnected', { groupId: this.groupId });
    }
